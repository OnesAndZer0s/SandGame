namespace Sandbox.Common.Maths.Shapes
{
  public class TriPrism
  {
    ...
    public TriPrism(double width = 1, double height = 1, double depth = 1)
    {
      BaseWidth = width;
      BaseHeight = height;
      Height = depth;
    ...
    {
      return (1 / 2) * BaseWidth * BaseHeight * Height;
    }
    ...
    {
      return Math.Sqrt((2 * volume) / (height * depth));
    }
    ...
    {
      return Math.Sqrt((2 * volume) / (width * depth));
    }
